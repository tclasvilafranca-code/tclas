# -*- coding: utf-8 -*-
"""A comme amour — pieza 17 de Eduard. Formato ADULTO. RETO.

   Lo comprobado sobre el PDF de su carpeta (Richard Clayderman,
   musicaparadisfrutar.com, 1 página; el mismo archivo que la pieza 17 de
   ...
   supiera dibujarlas, y dejo de serlo hace tiempo: la pieza esta en el bloque
   de los retos PRECISAMENTE por las semicorcheas y el alumno no las veia ni
   una vez. Medido sobre el PDF: 45 pares de barras dobles. El trabajo por
   grupos de cuatro sigue en corcheas, que es como se estudia un pasaje
   rapido, y ademas hay un sistema con la figura de verdad.
"""
import os
import sys
...
from ed_comun import (n, ac, sil, corch, semi, objetivo, plan, diferencias, figuras,
...
CANCION = dict(
    ...
    ficha=dict(
        ...
        pie_ritmos='Andamio en Mi menor: el dibujo es el de tu partitura y las notas exactas están '
                   'allí. Aquí van escritas en corcheas a propósito, para tocarlas al doble de '
                   'lento, que es como hay que estudiar un pasaje rápido.',
        armonia=dict(
        ...
        ritmos=[
            ('MANO DERECHA', 'las rápidas, escritas al doble de lento',
             corch(['F#4', 'E4']) + corch(['D4', 'C4']) + corch(['B3', 'C4']) +
             corch(['D4', 'F#4']), OCRE, 'treble', MIM),
            ('MANO IZQUIERDA', 'notas largas, sostiene y ya está · andamio',
             [ac(('B2', 'F#3'), 'w')], AZUL, 'bass', MIM),
        ],
        especial=[
    ...
    piano1=dict(
            ...
            dict(num=1, titulo='Grupos de cuatro, parando entre uno y otro',
                 ...
                 sistemas=[
                     dict(cap='a) cuatro notas, y para · la mano se queda floja mientras esperas',
                          events=corch(['B3', 'C4']) + corch(['D4', 'E4']) + [sil('h')] +
                                 corch(['F#4', 'E4']) + corch(['D4', 'C4']) + [sil('h')],
                          bars=2, key_sig=MIM),
                     dict(cap='b) y ahora dos grupos seguidos, sin parar en medio',
                          events=corch(['B3', 'C4']) + corch(['D4', 'E4']) +
                                 corch(['F#4', 'E4']) + corch(['D4', 'C4']),
            ...
            dict(num=2, titulo='La igualdad: el mismo grupo, muchas veces',
                 ...
                 sistemas=[
                     ...
                     dict(cap='b) y AHORA con la figura que vas a leer de verdad: la semicorchea · '
                          ...
                          events=semi(['B3', 'C4', 'D4', 'E4']) + semi(['F#4', 'E4', 'D4', 'C4']) +
                                 [n('B3'), n('F#4')],
                          bars=1, key_sig=MIM, show_time=False),
                 ]),
        ...
        ] + bloques_extra('Mi menor', 34, 'E4', 'E2',
    ...
    trabajo=[
        dict(
            ...
            bloques=[
                ...
                diferencias(
                    corch(['B3', 'C4']) + corch(['D4', 'E4']) + corch(['F#4', 'E4']),
                    corch(['B3', 'C4']) + corch(['D4', 'F#4']) + corch(['F#4', 'D4']),
                    cuantas=2,
