# -*- coding: utf-8 -*-
"""Oh, When the Saints — pieza 2 de Isaac.

   Lo comprobado sobre el PDF de su carpeta de Drive ("Oh, When the Saints",
   Primer Level, arr. Gilbert DeBenedetti, 1 página; el mismo archivo que la
   pieza 2 de Mercè y de Arnau, byte a byte):

     - Detrás de la clave NO HAY NADA: Do mayor.
     - 4/4, y arriba pone "Lively".
     - La melodía lleva los dedos escritos y letra debajo de cada nota.
...
CANCION = dict(
    alumno='Isaac', carpeta='Isaac', num=2, nivel='intermedio', slug='OhWhenTheSaints',
    formato='adulto',
    titulo_corto='Oh, When the Saints', time_sig=(4, 4), key_sig=None,
    partitura=os.path.join(HERE, '..', 'students', 'isaac', 'source', 'OH WHEN THE SAINT.pdf'),
    ...
    ficha=dict(
        ...
        truco='Toca solo la izquierda contando "UNO (silencio), dos, tres, cuatro" en voz alta, '
              'marcando el "uno" con la cabeza aunque no suene nada.',
    ...
    piano1=dict(
        ...
        bloques=[
            dict(num=1, titulo='La izquierda: silencio y tres negras',
                 ...
                 sistemas=[
                     dict(cap='a) callas el primer tiempo, con otro par de acordes',
                          events=[sil('q'), n('D3'), n('D3'), n('D3'),
                                  sil('q'), n('A2'), n('A2'), n('A2')],
                          bars=2, clef='bass'),
                     dict(cap='b) y otro par distinto, el silencio siempre en el mismo sitio',
                          events=[sil('q'), n('E3'), n('E3'), n('E3'),
                                  sil('q'), n('G2'), n('G2'), n('G2')],
                          bars=2, clef='bass', show_time=False),
                 ]),
            dict(num=2, titulo='La derecha: negras y una redonda final',
                 pista='andamio · la frase acaba siempre en una nota larga',
                 sistemas=[
                     dict(cap='a) empezando más arriba, bajando hasta la redonda',
                          events=[n('G4'), n('F4'), n('E4'), n('D4'), n('G4', 'w')],
                          bars=2),
                     dict(cap='b) y la frase siguiente, subiendo otra vez',
                          events=[n('A4'), n('B4'), n('C5'), n('D5'), n('A4', 'w')],
            ...
            dict(num=3, titulo='Las dos juntas',
                 ...
                 sistemas=[
                     dict(cap='a) la derecha entra sola donde calla la izquierda, con otro dibujo',
                          events=[n('G4'), ac(('D3', 'F4')), ac(('D3', 'E4')), ac(('D3', 'D4')),
                                  ac(('D3', 'G4'), 'w')],
                          bars=2),
                     dict(cap='b) y con la frase que baja',
                          events=[n('A4'), ac(('A2', 'G4')), ac(('A2', 'F4')), ac(('A2', 'E4')),
                                  ac(('A2', 'A4'), 'w')],
                          bars=2, show_time=False),
                 ]),
        ] + bloques_extra('Do mayor', 19, 'C4', 'C3',
                          'la melodía sube por grados sin ayuda del brazo',
