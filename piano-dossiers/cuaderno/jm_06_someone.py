# -*- coding: utf-8 -*-
"""Someone You Loved — pieza 6 de José María. Formato ADULTO.

   Lo comprobado sobre el PDF de su carpeta de Drive (Lewis Capaldi,
   Campamento Musical Bye Bye Beethoven, 3 páginas):
   ...
   Lo que NO se cita nota a nota: las corcheas van muy juntas y la medición
   de cabezas no sale limpia. El material melódico va como ANDAMIO en Do
   mayor y remite a la partitura.
"""
import os
import sys
...
from jm_comun import (n, ac, sil, corch, plan, metronomo, nombres, colorear,
                      escribir, para_clase)
...
CANCION = dict(
    alumno='José María', carpeta='JoseMaria', num=6, nivel='iniciación',
    ...
    ficha=dict(
        ...
        ritmos=[
            ('MANO DERECHA', 'corcheas seguidas, sin huecos · andamio',
             corch(['C5', 'B4']) + corch(['A4', 'G4']) + corch(['F4', 'G4']) +
             corch(['A4', 'B4']), OCRE, 'treble', None),
            ...
            dict(num=2, titulo='La izquierda: redondas, y luego dos a la vez', clef='bass',
                 ...
                 sistemas=[
                     dict(cap='a) una sola nota por compás, los cuatro primeros',
                          events=[n('A2', 'w'), n('F2', 'w'), n('C3', 'w'), n('G2', 'w')],
                          bars=4, clef='bass'),
                     dict(cap='b) y del c. 5 en adelante, dos a la vez · bajan juntas',
                          events=[ac(('A2', 'E3'), 'w'), ac(('F2', 'C3'), 'w'),
                                  ac(('C3', 'G3'), 'w'), ac(('G2', 'D3'), 'w')],
