# -*- coding: utf-8 -*-
"""Shallow — pieza 10 de José María. Formato ADULTO.

   Lo comprobado sobre el PDF de su carpeta de Drive (Lady Gaga y Bradley
   Cooper, Campamento Musical Bye Bye Beethoven, 3 páginas):
   ...
   Lo que NO se cita nota a nota: la melodía va en corcheas con ligaduras y la
   medición no sale limpia. El material va como ANDAMIO en Sol mayor y remite
   a la partitura.
"""
import os
import sys
...
from jm_comun import (n, ac, sil, corch, plan, metronomo, ordenar, colorear,
                      nombres, para_clase)
...
CANCION = dict(
    alumno='José María', carpeta='JoseMaria', num=10, nivel='iniciación',
    ...
    ficha=dict(
        ...
        ritmos=[
            ('MANO DERECHA', 'entra en el dos, con dos corcheas · andamio',
             [sil('q')] + corch(['B3', 'D4']) + [n('G4'), n('B4')], OCRE, 'treble', SOL),
            ('MANO IZQUIERDA', 'dos notas largas, y aguantan · andamio',
             [ac(('G2', 'D3'), 'w')], AZUL, 'bass', SOL),
        ],
        especial=[
            ...
            dict(num=3, titulo='La izquierda, y las dos juntas', clef='bass',
                 ...
                 sistemas=[
                     dict(cap='a) la izquierda sola: entra en el uno y aguanta',
                          events=[ac(('G2', 'D3'), 'w'), ac(('E2', 'B2'), 'w'),
                                  ac(('C3', 'G3'), 'w'), ac(('D3', 'A3'), 'w')],
