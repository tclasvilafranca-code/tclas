# -*- coding: utf-8 -*-
"""Flying Theme (Cómo entrenar a tu dragón) — pieza 19 de José María.
   Formato ADULTO. EL ÚLTIMO RETO DEL CURSO.

   Lo comprobado sobre el PDF de su carpeta de Drive ("Flying Theme, adapted
   from How to Train Your Dragon", arreglo de Perfect Harmony, 3 páginas):
     ...
     - LAS DOS MANOS SE MUEVEN A LA VEZ y en corcheas: no hay una mano que
       sostenga mientras la otra corre, que es lo que había en casi todo el
       resto del curso.
     - Hay acordes de tres notas en la derecha y barras de repetición.
     - Los números de compás vienen impresos (6, 13, 21, 28).

   Es la última del curso a propósito: junta todo lo que se ha trabajado —dos
   manos ocupadas, corcheas seguidas, acordes, una tonalidad con dos
   sostenidos— y encima añade el cambio de tono. Si esta sale, el curso está
   hecho.
...
from jm_comun import (n, ac, sil, corch, objetivo, plan, verdadero_falso,
...
CANCION = dict(
    alumno='José María', carpeta='JoseMaria', num=19, nivel='iniciación',
    ...
    ficha=dict(
        ...
        ritmos=[
            ('MANO DERECHA', 'corcheas seguidas · andamio',
             corch(['G4', 'A4']) + corch(['C5', 'A4']) + corch(['G4', 'E4']) +
             corch(['D4', 'G4']), OCRE, 'treble', None),
            ...
            dict(num=3, titulo='Los acordes de la derecha',
                 ...
                 sistemas=[
                     dict(cap='a) los tres dedos bajan juntos, y la mano no se tensa entre acorde y '
                              'acorde',
                          events=[ac(('C4', 'E4', 'G4'), 'h'), ac(('D4', 'F4', 'A4'), 'h'),
                                  ac(('E4', 'G4', 'C5'), 'h'), ac(('C4', 'E4', 'G4'), 'h')],
                          bars=2),
                     dict(cap='b) y con el acorde y la melodía en la misma mano · el acorde no puede '
                              'atropellar a la nota que va suelta',
                          events=[ac(('C4', 'E4', 'G4'), 'h'), n('G4'), n('E4'),
                                  ac(('D4', 'F4', 'A4'), 'h'), n('A4'), n('F4')],
    ...
    trabajo=[
        dict(
            ...
            bloques=[
                ...
                dibujar(['Do', 'Re', 'Mi', 'Fa♯', 'Sol', 'La', 'Si', 'Do♯'],
