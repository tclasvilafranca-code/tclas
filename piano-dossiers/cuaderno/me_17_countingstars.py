# -*- coding: utf-8 -*-
"""Counting Stars — pieza 17 de Mercè.

   Lo comprobado sobre el PDF de su carpeta de Drive (OneRepublic, arr. Becky
   Messer, *Easy Version*, 2 páginas; el mismo archivo que la pieza 4 de José
   María y la pieza 4 de Josep, byte a byte), leído a 230 dpi:

     - Detrás de la clave NO HAY NADA: Do mayor.
     - 4/4.
     - La izquierda toca redondas de dos notas a la vez, una por compás, con
       varias ligadas al compás siguiente.
...
CANCION = dict(
    alumno='Mercè', carpeta='Merce', num=17, nivel='intermedio', slug='CountingStars',
    formato='adulto',
    titulo_corto='Counting Stars', time_sig=(4, 4), key_sig=None,
    partitura=os.path.join(HERE, '..', 'students', 'merce', 'source', 'counting-stars-.pdf'),
    ...
    ficha=dict(
        ...
        armonia=dict(
            ...
            tarjetas=[
                ('REDONDAS LIGADAS', 'Cruzan el compás',
                 ...
                 'sigue sonando en el compás siguiente sin volver a tocarla.'),
                ('DIGITACIÓN OBLIGADA', 'Impresa nota a nota',
                 'Con tanta corchea seguida, seguir los dedos exactos que trae la partitura es lo '
                 'que evita que la mano se enrede.'),
                ('DOS NOTAS A LA VEZ', 'En la izquierda',
                 'Cada redonda es en realidad un intervalo de dos notas, no una sola: ambas tienen '
                 'que sonar juntas y sostenerse igual.'),
                ('POCOS ACORDES', 'Que se repiten',
                 'La izquierda usa muy pocas combinaciones distintas en toda la pieza, así que '
                 'aprenderlas bien de entrada ahorra trabajo después.'),
            ],
            pie='Es una canción de 2013 muy popular, y esta versión reduce la melodía a corcheas '
        ...
        especial=[
            ...
            'La izquierda usa pocas combinaciones distintas, que se repiten a lo largo de la pieza.',
        ],
        reto='Seguir la digitación impresa incluso cuando una mano "por libre" haría otra cosa. Con '
             'tantas corcheas, un dedo fuera de sitio se nota enseguida en la siguiente nota.',
        truco='Toca la derecha muy despacio, diciendo en voz alta el número de dedo antes de bajarlo, '
              'nota por nota. Cuando los dedos salgan solos sin decirlos, sube la velocidad.',
        sabias='La canción llegó al número uno en varios países en 2013, y su título hace referencia '
               'a contar estrellas en vez de ovejas para no poder dormir, pensando en alguien.',
        qr=dict(titulo='Escúchala',
                texto='Escucha la base de acordes largos bajo la melodía. Fíjate en que algunos '
                      'duran más de un compás sin que se note el cambio.'),
    ),
        ...
        bloques=[
            ...
            dict(num=2, titulo='La derecha: corcheas con dedos fijos',
                 ...
                 sistemas=[
                     ...
                     dict(cap='b) el mismo dibujo, una tercera más arriba',
                          events=corch(['G4', 'A4', 'B4', 'A4']) + corch(['G4', 'A4', 'B4', 'D5']),
            ...
            dict(tipo='nota',
                 ...
                 texto='Una nota ligada al compás siguiente es la misma nota que sigue sonando: no '
                       'hay un segundo golpe. Volver a pulsarla, aunque suene parecido, corta el '
                       'sonido y rompe el fondo continuo que sostiene la melodía por encima. Se toca '
                       'una vez y se deja vivir hasta que le corresponda cambiar de verdad.'),
