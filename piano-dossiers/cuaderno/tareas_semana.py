...
"""La tarea escrita de la semana, para Dilan y Eva.
   ...
   Cada tarea lleva tres cosas: la etiqueta, el texto y **con qué se vuelve a
   la clase**. Esa tercera parte se añadió después, al darse cuenta de que
   José María tenía en su cuaderno un recuadro "para la próxima clase" y Dilan
   y Eva no, cuando los tres vienen a clase igual. La tarea de la semana no se
   acaba en casa: vuelve el día de la clase y se mira.
"""
...
TAREAS = [
    ('ANÁLISIS',
     ...
     'la partitura marcada, y cuántos compases nuevos te han salido'),
    ('DIGITACIÓN',
     ...
     'ese pasaje tocado con TU digitación, para revisarla entre los dos'),
    ('METRÓNOMO',
     ...
     'los siete números, y la pieza a la velocidad del último día'),
    ('DE MEMORIA',
     ...
     'esas cuatro líneas de memoria, con la partitura cerrada'),
    ('ESCUCHAR',
     ...
     'los nombres de las dos versiones y la diferencia que has encontrado'),
    ('RITMO SOLO',
     ...
     'las notas.',
     'ese compás palmeado, y luego tocado, para ver si era el ritmo'),
    ('DINÁMICA',
     ...
     'tus marcas y tus razones · se discuten, no se corrigen'),
    ('TRANSPORTAR',
     ...
     'enteras de nada.',
     'la hoja escrita, y esas dos líneas tocadas en el tono nuevo'),
    ('PRIMERA VISTA',
     ...
     'la lista de lo que te ha fallado cada día · ahí se ve el patrón'),
    ('COPIAR',
     ...
     'el compás copiado, y qué has visto al copiarlo que no habías visto'),
    ('CANTAR',
     ...
     'de oído.',
     'cantarla y tocar la izquierda a la vez, aunque desafines'),
    ('EL PEDAL',
     ...
     'la partitura con los cambios marcados, para probarlos en el piano bueno'),
]
...
def tarea(alumno, num):
    ...
    etiqueta, texto, clase = TAREAS[i]
    return dict(etiqueta=etiqueta, texto=texto, clase=clase)
