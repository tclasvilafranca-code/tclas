...
"""Taller de practica - Do Your Ears Hang Low (Arnau, cancion 4, Do
   mayor, 4/4). Nivel iniciacion: el salto gracioso, subir y bajar
...
def page1(c):
    ...
    y = exercise_heading(c, y, 1, 'Calentamiento: escalón a escalón', 1,
                          'Sube y baja la posición de 5 dedos con calma, escuchando cada nota.')
    y -= 9
    ...
    y = exercise_heading(c, y, 2, 'El salto gracioso: sube y baja como orejas', 2,
                          'La melodía brinca de una nota grave a una aguda y vuelve — como orejas de perro balanceándose.')
    y -= 9
    ev2a = [{'pitch': p, 'dur': 'q'} for p in ['E4', 'C5', 'E4', 'C5', 'F4', 'D5', 'F4', 'D5']]
    y = system_block(c, x0, w0, y, gap, 'a) El brinco que se balancea', ev2a, clef='treble', time_sig=TS)

    ev2b = [{'pitch': p, 'dur': 'q'} for p in ['E4', 'F4', 'G4', 'A4', 'B4', 'A4', 'G4', 'F4']]
    y = system_block(c, x0, w0, y, gap, 'b) El mismo camino, ahora paso a paso: compáralo', ev2b, clef='treble', time_sig=TS)

    treb2c = [{'pitch': p, 'dur': 'q'} for p in ['E4', 'C5', 'E4', 'C5', 'F4', 'D5', 'F4', 'D5']]
    bass2c = [{'pitches': DO, 'dur': 'w'}] * 2
    y = grand_staff_block(c, x0, w0, y, gap, treb2c, bass2c, 'c) El brinco sobre el acorde de Do, quieto', grand_gap_mult=7.3, time_sig=TS)
    y -= 4
...
def page2(c):
    ...
    y = grand_staff_block(c, x0, w0, y, gap, treb1, bass1, 'a) El brinco sobre Fa, quieto', grand_gap_mult=7.3, time_sig=TS)
    ...
    y -= 7
    treb3 = [{'pitch': p, 'dur': 'q'} for p in ['G4', 'E5', 'G4', 'E5', 'A4', 'F5', 'A4', 'F5']]
    bass3 = [{'pitches': SOL, 'dur': 'w'}] * 2
    y = grand_staff_block(c, x0, w0, y, gap, treb3, bass3, 'a) El brinco vuela; el acorde de Sol no se mueve', grand_gap_mult=7.3, time_sig=TS)
