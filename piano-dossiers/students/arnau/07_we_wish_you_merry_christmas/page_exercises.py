...
"""Taller de practica - We Wish You a Merry Christmas (Arnau, cancion
   7, Do mayor, 3/4). Formato por BLOQUES del Dosier Exhaustivo de
...
def page1(c):
    ...
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Un villancico clásico en Do mayor, en 3/4. Hoy: la izquierda "responde" a la derecha.')
    y -= 15
    gap = 7.3
    ...
    y = bullet_list(c, y, [
        ...
        'Rotaciones suaves de muñeca, en el aire y sobre la tapa cerrada.',
        'Cuenta "uno-dos-tres" balanceando el cuerpo, como un vals lento.',
    ], dot_color=BLOQUE_COLOR[1])
    y -= 8

    y = bloque_heading(c, y, 2, 'Ya en el piano. Hoy: la derecha pregunta, la izquierda responde con calma.')
    y -= 4
    ...
    y = system_block(c, x0, w0, y, gap, 'b) La respuesta del bajo, sola (la dificultad de hoy)', ev2b, clef='bass', time_sig=TS)
...
def page2(c):
    ...
    c.setFont('DejaVuSans', 9.2)
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Categoría E: imagina una conversación — la derecha pregunta, la izquierda contesta bajito.')
    y -= 15
    gap = 6.9
    x0, w0 = MARGIN, CONTENT_W

    y = bloque_heading(c, y, 3, 'Con la partitura al lado. Pregunta y respuesta, sin adelantarse.')
    y -= 4
    treb1 = [{'pitch': p, 'dur': 'h.'} for p in ['A4', 'F4', 'C4']]
    bass1 = [{'pitches': FA, 'dur': 'h.'}] * 3
    y = grand_staff_block(c, x0, w0, y, gap, treb1, bass1, 'a) Manos juntas: pregunta y respuesta sobre Fa', grand_gap_mult=6.8, time_sig=TS)

    treb5 = ([{'pitch': p, 'dur': 'h.'} for p in ['G4', 'E4', 'C4']] +
             [{'pitch': p, 'dur': 'h.'} for p in ['A4', 'F4', 'C4']])
    bass5 = [{'pitches': p, 'dur': 'h.'} for p in [DO, DO, DO, FA, FA, FA]]
    y = grand_staff_block(c, x0, w0, y, gap, treb5, bass5, 'b) Reto extra: We Wish You a Merry Christmas casi entera', grand_gap_mult=6.8, time_sig=TS)
    y -= 6

    y = bloque_heading(c, y, 4, 'De oído. El profesor toca, Arnau responde en voz alta (no se escribe).')
    y -= 2
    y = bullet_list(c, y, [
        'Toca la pregunta (derecha) y la respuesta (izquierda): ¿es la misma nota grave siempre?',
        'Toca el acorde de Do y luego el de Fa: ¿cuál es más grave?',
        'Canta la primera frase antes de tocarla: ¿coincide con las notas reales?',
