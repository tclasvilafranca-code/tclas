...
"""Taller de practica - The Wheels on the Bus (Arnau, cancion 10, Fa
   ...
   Ejercicios de Piano (bloques 1,2,3,4,6). Primera tonalidad con
   armadura (Sib)."""
...
def page1(c):
    ...
    y = bullet_list(c, y, [
        ...
        'Localiza en el aire el grupo de "2 teclas negras" — ahí vive el Sib de hoy.',
        'Camina los dedos sobre la tapa cerrada, dedo 1-2-3-4-5.',
    ], dot_color=BLOQUE_COLOR[1])
    y -= 8

    y = bloque_heading(c, y, 2, 'Ya en el piano. Hoy: posición de Fa mayor y la rueda que gira, nota repetida.')
    y -= 4
    ...
    y = system_block(c, x0, w0, y, gap, 'b) La rueda que gira: nota repetida (la dificultad de hoy)', ev2a, clef='treble', time_sig=TS, key_sig=KEY)
...
def page2(c):
    ...
    c.setFont('DejaVuSans', 9.2)
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Categoría E: aísla el Sib — tócalo solo, varias veces, hasta que el dedo lo encuentre sin dudar.')
    y -= 15
    gap = 6.9
    x0, w0 = MARGIN, CONTENT_W

    y = bloque_heading(c, y, 3, 'Con la partitura al lado. El acorde de Sib queda quieto; la rueda gira encima.')
    y -= 4
    treb1 = [{'pitch': p, 'dur': 'q'} for p in ['D5', 'D5', 'D5', 'D5', 'C5', 'C5', 'Bb4', 'Bb4']]
    bass1 = [{'pitches': SIb, 'dur': 'w'}] * 2
    y = grand_staff_block(c, x0, w0, y, gap, treb1, bass1, 'a) Manos juntas: la rueda sobre Sib, quieto', grand_gap_mult=6.8, time_sig=TS, key_sig=KEY)
    ...
    y = grand_staff_block(c, x0, w0, y, gap, treb5, bass5, 'b) Reto extra: The Wheels on the Bus casi entera', grand_gap_mult=6.8, time_sig=TS, key_sig=KEY)
    y -= 6

    y = bloque_heading(c, y, 4, 'De oído. El profesor toca, Arnau responde en voz alta (no se escribe).')
    y -= 2
    y = bullet_list(c, y, [
        'Toca el Sib y luego el Si natural: ¿los distingues, aunque suenen parecido?',
        'Toca 4 Do seguidos: ¿suenan todos igual de fuerte?',
        'Toca el acorde de Fa y luego el de Sib: ¿cuál es más agudo?',
