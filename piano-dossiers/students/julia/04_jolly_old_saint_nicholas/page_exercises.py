# -*- coding: utf-8 -*-
"""Taller de practica - Jolly Old Saint Nicholas (Julia, cancion 4,
   Do mayor, 4/4). Nivel inicial: el paso a paso -- subir y bajar la
   escalera de notas, sin saltarse ningun escalon."""
from page_layout_common import *

SONG_KICKER = 'JULIA · NIVEL INICIAL · JOLLY OLD SAINT NICHOLAS'
TS = (4, 4)
...
def page1(c):
    ...
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Un villancico en Do mayor. Hoy subimos y bajamos la escalera de notas, ¡sin saltarnos ningún escalón!')
    y -= 15
    gap = 7.6
    ...
    y = exercise_heading(c, y, 2, 'El paso a paso: la escalera de notas', 2,
                          'Lo que vamos a practicar hoy. La melodía sube y baja escalón a escalón, sin saltarse ninguno — como subir una escalera con cuidado.')
    y -= 9
    ev2a = [{'pitch': p, 'dur': 'q'} for p in ['C4', 'D4', 'E4', 'F4', 'G4', 'A4', 'G4', 'F4']]
    y = system_block(c, x0, w0, y, gap, 'a) La escalera, subiendo y bajando', ev2a, clef='treble', time_sig=TS)
