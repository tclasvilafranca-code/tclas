# -*- coding: utf-8 -*-
"""Taller de practica - I Have a Dream (Luisa, cancion 11, Abba,
   Do mayor, 4/4). Nivel hobby, sin complicaciones: la cancion con
   esperanza -- solo hay que sonreir mientras tocas."""
from page_layout_common import *

SONG_KICKER = 'LUISA · NIVEL HOBBY · I HAVE A DREAM'
TS = (4, 4)
...
def page1(c):
    ...
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'I Have a Dream, de Abba, en Do mayor. Hoy: una canción con esperanza, solo hay que sonreír.')
    y -= 15
    gap = 7.6
    ...
    y = exercise_heading(c, y, 2, 'La canción con esperanza: solo hay que sonreír', 2,
                          'Lo de hoy. Esta canción habla de un sueño, con esperanza: no hay que complicarse, solo tocarla con una sonrisa.')
    y -= 9
    ev2a = [{'pitch': p, 'dur': 'q'} for p in ['C4', 'D4', 'E4', 'E4', 'D4', 'E4', 'F4', 'F4']]
    y = system_block(c, x0, w0, y, gap, 'a) La melodía sube, con esperanza', ev2a, clef='treble', time_sig=TS)

    ev2b = [{'pitch': p, 'dur': 'q'} for p in ['G4', 'F4', 'E4', 'D4', 'C4', 'D4', 'E4', 'D4']]
    y = system_block(c, x0, w0, y, gap, 'b) Y baja tranquila, sin prisa', ev2b, clef='treble', time_sig=TS)
...
def page2(c):
    ...
    c.setFont('DejaVuSans', 9.2)
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Ahora manos juntas, sin agobios: sonríe mientras la melodía sube con esperanza.')
    y -= 15
    gap = 7.1
    x0, w0 = MARGIN, CONTENT_W

    y = exercise_heading(c, y, 4, 'Manos juntas · la melodía sobre el acorde de Fa', 2,
                          'La izquierda sostiene el acorde entero, tranquila; la derecha canta encima, con esperanza.')
    y -= 7
    ...
    y = exercise_heading(c, y, 5, 'Independencia · sin agobios, cada mano tranquila', 3,
                          'La izquierda sostiene su acorde sin moverse; la derecha canta con esperanza, sin arrastrar a la de abajo.')
    y -= 7
    ...
    y = exercise_heading(c, y, 6, 'Reto extra · I Have a Dream casi entera', 3,
                          'Con la partitura al lado: sonríe, sin agobios, y disfruta la esperanza de la melodía.')
    y -= 7
    treb5 = ([{'pitch': p, 'dur': 'q'} for p in ['C4', 'D4', 'E4', 'E4']] +
             [{'pitch': p, 'dur': 'q'} for p in ['F4', 'G4', 'A4', 'A4']])
    bass5 = [{'pitches': p, 'dur': 'w'} for p in [DO, FA]]
    y = grand_staff_block(c, x0, w0, y, gap, treb5, bass5, 'I Have a Dream casi completa · sonriendo, sin agobios', grand_gap_mult=7.3, time_sig=TS)
