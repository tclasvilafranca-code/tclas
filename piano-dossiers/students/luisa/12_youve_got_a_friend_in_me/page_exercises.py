# -*- coding: utf-8 -*-
"""Taller de practica - You've Got a Friend in Me (Luisa, cancion
   12, Toy Story, Do mayor, 4/4). Nivel hobby, sin complicaciones:
   una cancion de amistad -- tocala pensando en alguien querido."""
from page_layout_common import *

SONG_KICKER = 'LUISA · NIVEL HOBBY · YOU\'VE GOT A FRIEND IN ME'
TS = (4, 4)
...
def page1(c):
    ...
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, "You've Got a Friend in Me, de Toy Story, en Do mayor. Hoy: piensa en un amigo mientras tocas.")
    y -= 15
    gap = 7.6
    ...
    y = exercise_heading(c, y, 2, 'Una canción de amistad: piensa en alguien querido', 2,
                          'Lo de hoy. Esta canción habla de la amistad: mientras tocas, piensa en alguien a quien quieras, y deja que la música suene tranquila.')
    y -= 9
    ev2a = [{'pitch': p, 'dur': 'q'} for p in ['C4', 'E4', 'G4', 'C5', 'G4', 'E4', 'C4', 'E4']]
    y = system_block(c, x0, w0, y, gap, 'a) La melodía tranquila, con cariño', ev2a, clef='treble', time_sig=TS)

    ev2b = [{'pitch': p, 'dur': 'q'} for p in ['G4', 'B4', 'D5', 'G5', 'D5', 'B4', 'G4', 'B4']]
    y = system_block(c, x0, w0, y, gap, 'b) La misma idea, un poco más arriba', ev2b, clef='treble', time_sig=TS)
...
def page2(c):
    ...
    c.setFont('DejaVuSans', 9.2)
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Ahora manos juntas, sin agobios: disfruta esta canción de amistad.')
    y -= 15
    gap = 7.1
    x0, w0 = MARGIN, CONTENT_W

    y = exercise_heading(c, y, 4, 'Manos juntas · la melodía sobre el acorde de Fa', 2,
                          'La izquierda sostiene el acorde entero, tranquila; la derecha canta encima, con cariño.')
    y -= 7
    ...
    y -= 7
    treb3 = [{'pitch': p, 'dur': 'q'} for p in ['D4', 'F4', 'A4', 'D5']]
    ...
    treb4 = [{'pitch': p, 'dur': 'q'} for p in ['D5', 'A4', 'F4', 'D4']]
    ...
    y = exercise_heading(c, y, 6, 'Reto extra · You\'ve Got a Friend in Me casi entera', 3,
                          'Con la partitura al lado: disfruta, sin agobios, esta canción de amistad.')
    y -= 7
    treb5 = ([{'pitch': p, 'dur': 'q'} for p in ['C4', 'E4', 'G4', 'C5']] +
             [{'pitch': p, 'dur': 'q'} for p in ['F4', 'A4', 'C5', 'F5']])
    bass5 = [{'pitches': p, 'dur': 'w'} for p in [DO, FA]]
    y = grand_staff_block(c, x0, w0, y, gap, treb5, bass5, "You've Got a Friend in Me casi completa · con cariño", grand_gap_mult=7.3, time_sig=TS)
