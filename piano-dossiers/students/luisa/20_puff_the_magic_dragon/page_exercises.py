# -*- coding: utf-8 -*-
"""Taller de practica - Puff the Magic Dragon (Luisa, cancion 20,
   arr. Eric Moore, Do mayor, 4/4). Nivel hobby, sin complicaciones:
   el dragon magico -- una cancion para imaginar."""
from page_layout_common import *

SONG_KICKER = 'LUISA · NIVEL HOBBY · PUFF THE MAGIC DRAGON'
TS = (4, 4)
...
def page1(c):
    ...
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Puff the Magic Dragon, en Do mayor. Hoy: el dragón mágico, una canción para imaginar.')
    y -= 15
    gap = 7.6
    ...
    y = exercise_heading(c, y, 2, 'El dragón mágico: una canción para imaginar', 2,
                          'Lo de hoy. Esta canción cuenta la historia de un dragón mágico: mientras tocas, imagina que vuelas con él, sin complicarte.')
    y -= 9
    ev2a = [{'pitch': p, 'dur': 'q'} for p in ['E4', 'D4', 'C4', 'D4', 'E4', 'F4', 'G4', 'E4']]
    y = system_block(c, x0, w0, y, gap, 'a) La melodía del dragón, con calma', ev2a, clef='treble', time_sig=TS)

    ev2b = [{'pitch': p, 'dur': 'q'} for p in ['E4', 'D4', 'C4', 'D4', 'E4', 'F4', 'G4', 'E4']]
    y = system_block(c, x0, w0, y, gap, 'b) La misma frase, muy despacio: para imaginar', ev2b, clef='treble', time_sig=TS)
...
def page2(c):
    ...
    c.setFont('DejaVuSans', 9.2)
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Ahora manos juntas, sin agobios: imagina al dragón mágico volando mientras tocas.')
    y -= 15
    gap = 7.1
    x0, w0 = MARGIN, CONTENT_W

    y = exercise_heading(c, y, 4, 'Manos juntas · la melodía sobre el acorde de Fa', 2,
                          'La izquierda sostiene el acorde de Fa, tranquila; la derecha canta encima, imaginando al dragón.')
    y -= 7
    ...
    y = exercise_heading(c, y, 6, 'Reto extra · Puff the Magic Dragon casi entera', 3,
                          'Con la partitura al lado: imagina al dragón mágico volando, sin agobios.')
    y -= 7
    treb5 = ([{'pitch': p, 'dur': 'q'} for p in ['E4', 'D4', 'C4', 'D4']] +
             [{'pitch': p, 'dur': 'q'} for p in ['A4', 'G4', 'F4', 'G4']])
    bass5 = [{'pitches': p, 'dur': 'w'} for p in [DO, FA]]
    y = grand_staff_block(c, x0, w0, y, gap, treb5, bass5, 'Puff the Magic Dragon casi completa · imaginando, sin agobios', grand_gap_mult=7.3, time_sig=TS)
