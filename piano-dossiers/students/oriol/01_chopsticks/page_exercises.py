# -*- coding: utf-8 -*-
"""Taller de practica - Chopsticks (Oriol, cancion 1, Level One,
   arr. Gilbert DeBenedetti, Do mayor, 3/4, mismo archivo que Julia).
   Nivel medio, sin agobiar: el pulso tranquilo -- mantener el
   tiempo, sin prisa."""
from page_layout_common import *

SONG_KICKER = 'ORIOL · NIVEL MEDIO · CHOPSTICKS'
TS = (3, 4)
...
def page1(c):
    ...
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Chopsticks, un clásico del piano, en Do mayor. Hoy: el pulso tranquilo, sin prisa.')
    y -= 15
    gap = 7.6
    ...
    y = exercise_heading(c, y, 2, 'El pulso tranquilo: mantener el tiempo, sin prisa', 2,
                          'Lo de hoy. Esta pieza famosa se toca fácilmente si mantienes un pulso constante, sin acelerarte por la emoción: cuenta siempre igual, del principio al final.')
    y -= 9
    ev2a = [{'pitch': p, 'dur': 'h.'} for p in ['C4', 'D4', 'E4']]
    y = system_block(c, x0, w0, y, gap, 'a) Nota larga, contando tres tiempos completos, sin prisa', ev2a, clef='treble', time_sig=TS)

    ev2b = [{'pitch': p, 'dur': 'h.'} for p in ['E4', 'D4', 'C4']]
    y = system_block(c, x0, w0, y, gap, 'b) Bajando, con el mismo pulso constante', ev2b, clef='treble', time_sig=TS)

    treb2c = [{'pitch': p, 'dur': 'h.'} for p in ['C4', 'D4', 'E4']]
    bass2c = [{'pitches': DO, 'dur': 'h.'}] * 3
    y = grand_staff_block(c, x0, w0, y, gap, treb2c, bass2c, 'c) La melodía sobre el acorde de Do, con el mismo pulso', grand_gap_mult=7.3, time_sig=TS)
    y -= 4

    y = exercise_heading(c, y, 3, 'Acordes I–IV–V en Do mayor', 2,
                          'Do–Fa–Sol: los tres acordes de esta tonalidad, sin más complicación.')
    y -= 11
...
def page2(c):
    ...
    c.setFont('DejaVuSans', 9.2)
    c.setFillColor(GRAY)
    c.drawString(MARGIN, y, 'Ahora manos juntas, sin agobios: mantén el mismo pulso tranquilo todo el tiempo.')
    y -= 15
    gap = 7.1
    x0, w0 = MARGIN, CONTENT_W

    y = exercise_heading(c, y, 4, 'Manos juntas · el pulso tranquilo sobre el acorde de Fa', 2,
    ...
    y = exercise_heading(c, y, 6, 'Reto final · Chopsticks casi entera', 3,
                          'Con la partitura al lado: mantén el pulso tranquilo, sin acelerarte, del principio al final.')
    y -= 7
    treb5 = ([{'pitch': p, 'dur': 'h.'} for p in ['C4', 'D4', 'E4']] +
             [{'pitch': p, 'dur': 'h.'} for p in ['F4', 'G4', 'A4']])
    bass5 = [{'pitches': p, 'dur': 'h.'} for p in [DO, DO, DO, FA, FA, FA]]
    y = grand_staff_block(c, x0, w0, y, gap, treb5, bass5, 'Chopsticks casi completa · pulso tranquilo, sin prisa', grand_gap_mult=7.3, time_sig=TS)
